...
window.addEventListener('DOMContentLoaded', () => {
	...
	registerBlockVariation('core/cover', {
		...
		description: __(
			'Cover preconfigurata come sezione hero con overlay scuro.',
			'sage',
		),
		category: 'theme',
		icon: 'cover-image',
	...
	registerBlockVariation('core/group', {
		...
		description: __(
			'Gruppo con sfondo, padding e bordo — ideale per card di contenuto.',
			'sage',
		),
		category: 'theme',
		icon: 'id-alt',
	...
	registerBlockVariation('core/group', {
		...
		description: __(
			'Gruppo a larghezza piena con sfondo ink e testo chiaro.',
			'sage',
		),
		category: 'theme',
		icon: 'shield',
			...
			el(
				...
				el(
					...
					el(MediaPanel, {
						...
						onRemove: () =>
							setAttributes({ authorImageId: 0, authorImageUrl: '' }),
					}),
				),
	...
	edit({ attributes, setAttributes }) {
		...
		return el(
			...
			el(
				'div',
				useBlockProps(),
				el(ServerSideRender, { block: 'theme/accordion', attributes }),
			),
		);
	},
	save: () => null,
...
registerBlockType('theme/products-carousel', {
	edit({ attributes, setAttributes }) {
		...
		const toggleCategory = (slug, checked) => {
			const next =
				checked ? [...categories, slug] : categories.filter((s) => s !== slug);
