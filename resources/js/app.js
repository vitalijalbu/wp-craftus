...
Alpine.data('siteHeader', () => ({
	...
	closeMenu() {
		...
		document
			...
			?.classList.remove('header-mega-open');
		if (!panel) {
...
Alpine.data('cartDrawer', () => ({
	...
	init() {
		...
		if (typeof jQuery !== 'undefined') {
			...
			jQuery(document.body).on('wc_fragment_refresh', () =>
				this.refreshFragment(),
			);
		}
	},
	...
	async submit() {
		...
		try {
			...
			if (res.ok && data.success) {
				...
				this.error =
					data.message ||
					(window.themeI18n?.genericError ?? 'Si è verificato un errore.');
			}
		} catch {
	...
	load() {
		try {
			const stored = JSON.parse(
				localStorage.getItem('theme:recently-viewed') || '[]',
			);
