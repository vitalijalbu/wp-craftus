@php
  ...
  $newsletter_heading = get_theme_mod('newsletter_heading', '');
...
<footer class="bg-dark text-white" role="contentinfo">
  ...
  {{-- ─── Main grid ───────────────────────────────────────────────────────── --}}
  <div class="container-fluid py-14 lg:py-20">
    <div class="grid grid-cols-2 lg:grid-cols-12 gap-10 lg:gap-6">
      ...
      <div class="col-span-2 lg:col-span-4">
        <a href="{{ esc_url(home_url('/')) }}" class="site-logo site-logo--footer block mb-5" aria-label="{{ esc_attr(get_bloginfo('name')) }}">
          @if($footer_logo_id)
            {!! wp_get_attachment_image($footer_logo_id, 'full', false, ['class' => 'custom-logo', 'alt' => get_bloginfo('name'), 'loading' => 'lazy', 'decoding' => 'async']) !!}
          @else
        ...
        @if($footer_tagline)
          <div class="leading-relaxed max-w-xs mb-8 text-sm text-white/70">
            {!! wp_kses_post($footer_tagline) !!}
          </div>
        @endif
      ...
      <div class="col-span-1 lg:col-span-2">
        ...
        <ul class="space-y-3">
          @foreach($shop_cats as $cat)
            <li><a href="{{ esc_url(get_term_link($cat)) }}">{{ esc_html($cat->name) }}</a></li>
          @endforeach
