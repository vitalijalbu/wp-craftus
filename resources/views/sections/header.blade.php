...
if (
        ...
        (string) wp_parse_url((string) wc_get_page_permalink('shop'), PHP_URL_PATH)
                ...
                <div class="flex items-center gap-4">
                    ...
                    @if (function_exists('WC'))
                        <a href="{{ esc_url(wc_get_page_permalink('myaccount')) }}" class="icon-btn"
                    ...
                    {{-- Cart --}}
                    @if (function_exists('WC'))
                        <button type="button" @click="$dispatch('open-cart')" class="icon-btn relative"
                        ...
                        @foreach ($item_children as $child)
                            <li>
                                <a href="{{ esc_url($child->url) }}"
                                    class="mega-item block text-sm text-ink/70 hover:text-primary transition-colors py-2">{{ esc_html($child->title) }}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
    ...
    {{-- ════════════════════════════════════════════════════════════════════════
       MOBILE DRAWER
       ════════════════════════════════════════════════════════════════════════ --}}
    <div id="mobile-drawer" x-show="mobileOpen" x-trap.inert.noscroll="mobileOpen"
        ...
        class="fixed inset-0 top-18 bg-ink z-40 overflow-y-auto flex flex-col lg:hidden" x-cloak role="dialog"
            ...
            @foreach ($top_items as $item)
                ...
                @if ($is_mega_mob && !empty($mob_children))
                    <div x-data="{ open: false }">
                        <button type="button" @click="open = !open" :aria-expanded="open.toString()"
                            ...
                            <span
                                class="font-sans text-lg font-medium text-white tracking-wide">{{ esc_html($item->title) }}</span>
                            <x-icons.chevron-down class="size-4 text-white/30 transition-transform duration-300"
                        ...
                        <div x-show="open" x-collapse class="py-3 space-y-1">
                            @foreach ($mob_children as $child)
                                <a href="{{ esc_url($child->url) }}"
                                    ...
                                    @click="closeMobile()">
                                    <span class="w-1 h-1 bg-primary/40 rounded-full shrink-0"
                ...
                @else
                    <a href="{{ esc_url($item->url) }}"
                        class="flex items-center justify-between py-5 border-b border-white/8 font-sans text-lg font-medium text-white hover:text-primary transition-colors tracking-wide"
                        @click="closeMobile()">{{ esc_html($item->title) }}</a>
                @endif
            @endforeach
        ...
        <div class="px-6 py-8 border-t border-white/8 space-y-4">
            @if ($show_cta)
                <a href="{{ esc_url($cta_url) }}"
                    ...
                    @click="closeMobile()">{{ esc_html($cta_label) }}</a>
            @endif
